 ...
 *
 */
static void provisioner_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id,
                                      void* event_data)
{
    ...
    if (event_base == NETWORK_PROV_EVENT) {
        ...
        case NETWORK_PROV_WIFI_CRED_FAIL: {
            ...
            // Log failure reason and reset provisioned credentials if too many retries have been
            // attempted
        ...
        if (device_config.wifi_power_save_mode == WULPUS_PRO_WIFI_PS_MIN_MODEM)
            power_save = WIFI_PS_MIN_MODEM;
        if (device_config.wifi_power_save_mode == WULPUS_PRO_WIFI_PS_MAX_MODEM)
            power_save = WIFI_PS_MAX_MODEM;
        ESP_ERROR_CHECK(esp_wifi_set_ps(power_save));

        // Signal provisioning task that we are connected, but only after the restart after
        // provisioning
 ...
 *
 */
static void itwt_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id,
                               void* event_data)
{
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_ITWT_SETUP: {
            ...
            if (setup->status == ITWT_SETUP_SUCCESS) {
                // /* TWT Wake Interval = TWT Wake Interval Mantissa * (2 ^ TWT Wake Interval
                // Exponent) */ ESP_LOGI(TAG, "<WIFI_EVENT_ITWT_SETUP>twt_id:%d, flow_id:%d, %s, %s,
                // wake_dura:%d, wake_dura_unit:%d, wake_invl_e:%d, wake_invl_m:%d",
                // setup->config.twt_id,
                //          setup->config.flow_id, setup->config.trigger ? "trigger-enabled" :
                //          "non-trigger-enabled", setup->config.flow_type ? "unannounced" :
                //          "announced", setup->config.min_wake_dura,
                //          setup->config.wake_duration_unit, setup->config.wake_invl_expn,
                //          setup->config.wake_invl_mant);
                // ESP_LOGI(TAG, "<WIFI_EVENT_ITWT_SETUP>target wake time:%lld, wake duration:%d us,
                // service period:%d us", setup->target_wake_time, setup->config.min_wake_dura <<
                // (setup->config.wake_duration_unit == 1 ? 10 : 8),
                //          setup->config.wake_invl_mant << setup->config.wake_invl_expn);
                ESP_LOGI(TAG, "TWT Duration: %d us",
                         setup->config.min_wake_dura
                             << (setup->config.wake_duration_unit == 1 ? 10 : 8));
                ESP_LOGI(TAG, "TWT Interval: %d us",
                         setup->config.wake_invl_mant << setup->config.wake_invl_expn);
                ...
                if (setup->status == ESP_ERR_WIFI_TWT_SETUP_TIMEOUT) {
                    ESP_LOGE(TAG,
                             "<WIFI_EVENT_ITWT_SETUP>twt_id:%d, timeout of receiving twt setup "
                             "response frame",
                             setup->config.twt_id);
                } else if (setup->status == ESP_ERR_WIFI_TWT_SETUP_TXFAIL) {
                    ESP_LOGE(
                        TAG,
                        "<WIFI_EVENT_ITWT_SETUP>twt_id:%d, twt setup frame tx failed, reason: %d",
                        setup->config.twt_id, setup->reason);
                } else if (setup->status == ESP_ERR_WIFI_TWT_SETUP_REJECT) {
                    ESP_LOGE(TAG,
                             "<WIFI_EVENT_ITWT_SETUP>twt_id:%d, twt setup request was rejected, "
                             "setup cmd: %d",
                             setup->config.twt_id, setup->config.setup_cmd);
                } else {
                    ESP_LOGE(TAG, "<WIFI_EVENT_ITWT_SETUP>twt_id:%d, twt setup failed, status: %d",
        ...
        case WIFI_EVENT_ITWT_TEARDOWN: {
            ...
            // wifi_event_sta_itwt_teardown_t *teardown = (wifi_event_sta_itwt_teardown_t
            // *)event_data; ESP_LOGI(TAG, "<WIFI_EVENT_ITWT_TEARDOWN>flow_id %d%s",
        ...
        case WIFI_EVENT_ITWT_SUSPEND: {
            ...
            // ESP_LOGI(TAG, "<WIFI_EVENT_ITWT_SUSPEND>status:%d, flow_id_bitmap:0x%x,
...
esp_err_t provisioner_init(void)
{
    ...
    status = esp_event_handler_register(NETWORK_PROV_EVENT, ESP_EVENT_ANY_ID,
                                        &provisioner_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering provisioning event handler %d", status);
        return status;
    }
    status =
        esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &provisioner_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering Wi-Fi event handler %d", status);
        return status;
    }
    status =
        esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &provisioner_event_handler, NULL);
    if (status != ESP_OK) {
    ...
    status =
        esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_ITWT_SETUP, &itwt_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering TWT event handler %d", status);
        return status;
    }
    status =
        esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_ITWT_TEARDOWN, &itwt_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering TWT event handler %d", status);
        return status;
    }
    status =
        esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_ITWT_SUSPEND, &itwt_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering TWT event handler %d", status);
        return status;
    }
    status =
        esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_ITWT_PROBE, &itwt_event_handler, NULL);
    if (status != ESP_OK) {
        ESP_LOGE(TAG, "Error registering TWT event handler %d", status);
        return status;
    }
...
static esp_err_t provisioner_process(void)
{
    ...
    if (!provisioned) {
        ...
        ESP_ERROR_CHECK(network_prov_mgr_start_provisioning(security, (const void*)sec_params,
                                                            service_name, service_key));

        // Wait for provisioning to finish
        xEventGroupWaitBits(provisioner_event_group, PROVISIONER_DONE_EVENT, false, true,
                            portMAX_DELAY);
...
esp_err_t provisioner_start(bool reset)
{
    ...
    xEventGroupClearBits(provisioner_event_group, PROVISIONER_DONE_EVENT |
                                                      PROVISIONER_CONNECTED_EVENT |
                                                      PROVISIONER_DISCONNECTED_EVENT);
...
esp_err_t provisioner_wait(void)
{
    ...
    xEventGroupWaitBits(provisioner_event_group, PROVISIONER_DONE_EVENT, false, true,
                        portMAX_DELAY);
    xEventGroupWaitBits(provisioner_event_group, PROVISIONER_CONNECTED_EVENT, false, true,
                        portMAX_DELAY);
...
esp_err_t provisioner_wait_disconnected(void)
{
    if (provisioner_event_group == NULL)
        return ESP_ERR_INVALID_STATE;
    xEventGroupWaitBits(provisioner_event_group, PROVISIONER_DISCONNECTED_EVENT, true, true,
                        portMAX_DELAY);
    return ESP_OK;
}
...
esp_err_t provisioner_get_status(wulpus_pro_wifi_status_t* status)
{
    ...
    *status =
        (wulpus_pro_wifi_status_t){.version = 1, .size = sizeof(*status), .state = wifi_state};
    ...
    if (wifi_state == WULPUS_PRO_WIFI_CONNECTED) {
        ...
        if (netif != NULL && esp_netif_get_ip_info(netif, &ip) == ESP_OK)
            memcpy(status->ipv4, &ip.ip.addr, 4);
    }
    return ESP_OK;
}
...
{
    if (!started || ssid == NULL || ssid_length == 0 || ssid_length > 32 || password_length > 64 ||
        (password_length && password == NULL))
        return ESP_ERR_INVALID_ARG;
...
esp_err_t provisioner_twt_setup(void)
{
    ...
    if (mode == WIFI_PHY_MODE_HE20) {
        ...
        // Set up TWT
        wifi_twt_setup_config_t config = {.setup_cmd = TWT_REQUEST,
                                          .flow_id = 0,
                                          .twt_id = 0,
                                          .flow_type = 0,
                                          .min_wake_dura = 10,
...
{
#if CONFIG_PROVISIONER_TWT_ENABLED
    if (!device_config.twt_enabled || wifi_state != WULPUS_PRO_WIFI_CONNECTED)
        return ESP_OK;
    esp_err_t status = ESP_OK;
