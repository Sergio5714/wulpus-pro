...
class WulpusProRxTxConfigGen:
    ...
    def add_config(self, tx_channels, rx_channels, optimized_switching=False):
        ...
        if any(channel > PRO_MAX_CH_ID for channel in tx_channels) or any(
            channel > PRO_MAX_CH_ID for channel in rx_channels
        ):
            raise ValueError(
                "RX and TX channel ID must be less than " + str(PRO_MAX_CH_ID)
            )
